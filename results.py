...
def getOneReplicate(combo, locus, rep):
    ...
    for line in rfile:
        if re.match("mean", line):
            ...
            uci = line.split()
    results = 'rep' + str(rep) + locus + " " + locus + "-" + str(rep) + " "
    if int(combo[3]) == 0:
        paramindex=[1,3,4,5,6,9,10,11,13]
        if locus == '0':
            loci = "10"
        elif locus == '1':
            loci = "100"
...
for combo in sim_combinations:
    if int(combo[3]) == 0:
        simSummary='ID theta_1A theta_1A_LCI theta_1A_UCI theta_3C theta_3C_LCI theta_3C_UCI theta_4R theta_4R_LCI theta_4R_UCI theta_5S theta_5S_LCI theta_5S_UCI	theta_6H theta_6H_LCI theta_6H_UCI tau_4R tau_4R_LCI tau_4R_UCI	tau_5S tau_5S_LCI tau_5S_UCI tau_6H tau_6H_LCI tau_6H_UCI phi_H phi_H_LCI phi_H_UCI noloci \n'
    else:
        simSummary='ID theta_1A theta_1A_LCI theta_1A_UCI theta_3C theta_3C_LCI theta_3C_UCI theta_4R theta_4R_LCI theta_4R_UCI theta_5S theta_5S_LCI theta_5S_UCI tau_4R tau_4R_LCI tau_4R_UCI tau_6H tau_6H_LCI tau_6H_UCI phi_H phi_H_LCI phi_H_UCI noloci \n'
    for locus in ('0','1','2'):
